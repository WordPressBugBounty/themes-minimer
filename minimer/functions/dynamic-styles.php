...
/* ------------------------------------ */
if ( ! function_exists( 'minimer_hex2rgb' ) ) {

	function minimer_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'minimer_enqueue_google_fonts' ) ) {

	function minimer_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'minimer_enqueue_google_fonts' ); 	


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'minimer_dynamic_css' ) ) {

	function minimer_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = minimer_hex2rgb($color_1);
...
.sidebar-skew-bottom,
.profile-skew-inner,
.profile-skew-inner-bottom { background: '.esc_attr( get_theme_mod('color-sidebar') ).'; }
				'."\n";
			}
			// header line color
			if ( get_theme_mod('color-header-line','#333333') != '#333333' ) {
				$styles .= '
#wrapper { border-top-color: '.esc_attr( get_theme_mod('color-header-line') ).'; }
				'."\n";
			}
			// header line height
			if ( get_theme_mod('header-line-height','10') != '10' ) {
				$styles .= '
#wrapper { border-top-width: '.esc_attr( get_theme_mod('header-line-height') ).'px; }
				'."\n";
			}
			// header color
...
.entry-header .entry-meta .entry-comments { color: '.esc_attr( get_theme_mod('color-comment-bubble') ).'; }
...
#footer-bottom .social-links .social-tooltip:hover:before { border-color: #fff transparent; }
			...
			wp_add_inline_style( 'minimer-style', $styles );	
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'minimer_dynamic_css' );
